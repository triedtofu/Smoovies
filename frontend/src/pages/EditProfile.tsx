...
import { apiGetUserDetails, apiUpdateUserDetails, UpdateUserParams } from '../util/api';
...
const EditProfile = () => {
  ...
  React.useEffect(() => {
    ...
    apiGetUserDetails(cookies.token)
      .then(data => {
        setName(data.name);
  ...
  const updatePassword = (e: React.FormEvent) => {
    ...
    if (newPassword !== confirmNewPasword) {
      setErrorStr2('New password and new password confirmation are not the same');
      setDisabledButton2(false);
      return;
    }

    const params: UpdateUserParams = { token: cookies.token, oldPassword, password: newPassword };
  ...
  return (
    <Container maxWidth="sm">
      ...
      <Typography gutterBottom variant="h4" component="h1">Edit Profile</Typography>

      <form onSubmit={updateDetails} className={styles.form}>
        {errorStr1 && <FormLabel sx={{ width: '100%' }} error={true}>{errorStr1}</FormLabel>}
        <TextField
          name="name"
          label="Name"
          size="small"
        ...
        <div className={styles.buttonDiv}>
          <Button variant="contained" type="submit" disabled={disabledButton1 || !newDetails()}>
            Update
          </Button>
        </div>
      </form>

      <Typography gutterBottom variant="h5" component="h2">Password</Typography>
      ...
      <form onSubmit={updatePassword} className={styles.form}>
        ...
        <div className={styles.buttonDiv}>
          <Button variant="contained" type="submit" disabled={disabledButton2 || !canSubmitPassword}>
            Update
          </Button>
        </div>
      </form>

      <Snackbar anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }} open={alertOpen} autoHideDuration={6000} onClose={handleAlertClose}>
